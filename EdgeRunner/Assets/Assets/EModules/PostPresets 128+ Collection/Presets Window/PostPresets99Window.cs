...
namespace EModules.PostPresets
{
  ...
  public partial class Params
  {
    ...
    public static List<ISupportedPostComponent> AllPostComponents = new List<ISupportedPostComponent>() { new PostPresets_UnityPostGUI_v2(), new PostPresets_DefaultFastMobile(),
...
#pragma warning restore
    ...
    void OnGUI()
    {
      ...
      if (!Camera.main) {
        GUILayout.Label( "Scene's Camera not found...", Params.Label );
        return;
      }

      //var cameraComponents = Camera.main.GetComponents<MonoBehaviour>().ToList();
      var assignedComponents = Params.AllPostComponents.Select((c,i)=>
      {
        if (c.SecondMonoComponentType != null) c.SecondMonoComponent = Camera.main.GetComponents(c.SecondMonoComponentType).Select(c2=>c2 as MonoBehaviour).OrderBy(c2=>c2.enabled ? 0 : 1).FirstOrDefault();
        return !AllPostComponentsInstalled[i] ? new MonoBehaviour[0] :  Camera.main.GetComponents(c.MonoComponentType).Select(f=>f as MonoBehaviour).Where(f=>f).ToArray() as MonoBehaviour[];
      } ).ToArray();
      ...
      var isp = EditorGUILayout.ToggleLeft( "Show Inspector", ShowInspector );
      ...
      if (ShowInspector) {
        GUILayout.BeginHorizontal();
        GUILayout.Label( "Inspector Component" );
      ...
      if (Params.AllPostComponents[NEW_POP_A].GET_TITLE != Params.selectedComponent ||
        !drawDots && Params.AllPostComponents[NEW_POP_O].GET_TITLE != Params.anotherComponentsToPreview ||
        NEW_USE != Params.useAnotherComponentsToPreview) {

        Params.selectedComponent.Set( Params.AllPostComponents[NEW_POP_A].GET_TITLE );
        Params.anotherComponentsToPreview.Set( Params.AllPostComponents[NEW_POP_O].GET_TITLE );
      ...
      GUILayout.Label( "Camera: " + Camera.main.name, Params.Label, GUILayout.Width( leftwidth ) );
      ...
      if (currentComponent.MonoComponent == null || currentComponent.SecondMonoComponentType != null && currentComponent.SecondMonoComponent == null) {
        ...
        if (GUILayout.Button( "Add " + compName + " Script", GUILayout.Height( 200 ) )) {
          Undo.RecordObject( Camera.main.gameObject, "Add " + compName + " Script" );
          if (currentComponent.MonoComponent == null) currentComponent.MonoComponent = Camera.main.gameObject.AddComponent( currentComponent.MonoComponentType ) as MonoBehaviour;
          if (currentComponent.SecondMonoComponent == null && currentComponent.SecondMonoComponentType != null) currentComponent.SecondMonoComponent = Camera.main.gameObject.AddComponent( currentComponent.SecondMonoComponentType ) as MonoBehaviour;
      ...
      if (!DRAW_DIF_PREV) {
        if (!currentComponent.LutEffectExist) {
          GUILayout.Label( "The component settings do not allow you to assign a LUT", Params.Label );
          return;
        }
        else {
    ...
    {
      return EditorGUILayout.GetControlRect( GUILayout.Width( Math.Min( (Mathf.Max( 100, window.position.width - leftWidth - 10 - RIGHTADD( window ) )) / divider, 700 / divider ) ), GUILayout.Height( height ) );
    }
