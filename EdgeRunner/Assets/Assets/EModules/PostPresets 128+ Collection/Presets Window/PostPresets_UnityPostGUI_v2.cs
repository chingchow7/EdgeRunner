...
namespace EModules.PostPresets
{
  class PostPresets_UnityPostGUI_v2 : ISupportedPostComponent
  {
    ...
    void ISupportedPostComponent.LeftSideGUI(EditorWindow window, float width)
    {
      ...
      if (!((ISupportedPostComponent)this).LutEffectExist) {
        ...
        if (GUILayout.Button( "enable LUT texture", GUILayout.Height( 200 ) )) {
          Undo.RecordObject( Camera.main.gameObject, "enable LUT texture" );
    ...
    PostProcessingModelFake AddModel(string name, ScriptableObject profile)
    {
      ...
      if (result == null) {
        if (AddSettings == null) AddSettings = profile.GetType().GetMethods( (BindingFlags)int.MaxValue ).FirstOrDefault( m => m.Name == "AddSettings" && m.GetParameters().Length == 1 && m.GetParameters()[0].ParameterType == typeof( Type ) );
